...
def make():
    ...
    operation_control = vr_task_logic.OperationControl(
        movable_spout_control=vr_task_logic.MovableSpoutControl(
            ...
            servo_motor=vr_task_logic.ServoMotor(
                min_pulse_duration=1000, max_pulse_duration=2000
            ),
        ),
        audio_control=vr_task_logic.AudioControl(),
        odor_control=vr_task_logic.OdorControl(
            valve_max_open_time=100000,
    ...
    def OperantLogicHelper(
        stop_duration: float = 0.2, is_operant: bool = False
    ):
    ...
    def Reward_VirtualSiteGeneratorHelper(contrast: float = 0.5):
        return vr_task_logic.VirtualSiteGenerator(
            render_specification=vr_task_logic.RenderSpecification(
                contrast=contrast
            ),
    ...
    patch1 = vr_task_logic.PatchStatistics(
        label="Amyl Acetate",
        state_index=0,
        odor_specification=vr_task_logic.OdorSpecification(
            index=1, concentration=1
        ),
    ...
    environment_statistics = vr_task_logic.EnvironmentStatistics(
        first_state=None,
        transition_matrix=vr_task_logic.Matrix2D(data=[[1, 0], [0, 1]]),
