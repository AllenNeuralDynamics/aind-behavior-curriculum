...
class DynamicForagingParas(abc.TaskParameters):
    ...
    SubTask: DynamicForagingSubTask = Field(..., title="Subtask of dynamic foraging")
    
    # Reward probability
    BaseRewardSum: float = Field(..., title="Sum of p_reward", ge=0.0, le=1.0)
    RewardFamily: int = Field(..., title="Reward family", ge=1)  # Should be explicit here
    RewardPairsN: int = Field(..., title="Number of pairs", ge=1)  # Should be explicit here
    ...
    BlockMinReward:  int = Field(1, title="Minimal rewards in a block to switch", ge=0)
    ...
    Multiplier: float = Field(..., title="Multiplier for auto reward", ge=0.0, le=1.0)
    Unrewarded: int = Field(..., title="Number of unrewarded trials before auto water", ge=0)
    Ignored: int = Field(..., title="Number of ignored trials before auto water", ge=0)
        
    # ITI
    ITIMin: float = Field(..., title="ITI (min)", ge=0.0)
    ITIMax: float = Field(..., title="ITI (max)", ge=0.0)
    ITIBeta: float = Field(..., title="ITI (beta)", ge=0.0)
    ITIIncrease: float = Field(0.0, title="ITI increase", ge=0.0)   # TODO: not implemented in the GUI??
    ...
    RewardConsumeTime: float = Field(..., title="Reward consume time", 
                                     ...
                                     ge=0.0)
    StopIgnores: int = Field(..., title="Number of ignored trials before stop", ge=0)
    
    # Auto block
    AdvancedBlockAuto: AdvancedBlockMode = Field(..., title="Auto block mode")    
    SwitchThr: float = Field(..., title="Switch threshold for auto block", ge=0.0, le=1.0)
    PointsInARow: int = Field(..., title="Points in a row for auto block", ge=0)
                             
    # Auto stop
    MaxTrial: int = Field(..., title="Maximal number of trials", ge=0)
    MaxTime: int = Field(..., title="Maximal session time (min)", ge=0)
                             
    RightValue_volume: float = Field(3.0, title="Right reward size (uL)", ge=0.0)
